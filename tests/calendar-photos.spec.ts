...
test('photo upload, review, persistence and private image access',async({page,browser})=>{
 ...
 await page.locator('input[name=username]').fill(username);await page.locator('input[name=password]').fill(password);await page.getByRole('button',{name:'アカウントを作成',exact:true}).click();await expect(page.getByRole('heading',{name:'今日の記録'})).toBeVisible();
 ...
 try{
  ...
  await page.getByRole('button',{name:'カレンダー',exact:true}).click();await page.getByRole('button',{name:'写真を追加'}).click();
  ...
  await page.reload();await page.getByRole('button',{name:'カレンダー',exact:true}).click();await expect(page.getByRole('heading',{name:'写真から記録したサラダ'})).toBeVisible();
